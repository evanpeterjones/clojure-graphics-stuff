(ns vulgaroomba.move
  (:require [vulgaroomba.utils :as utils]
...
(def ^:const acceleration -100)
(def ^:const gravity 500)
(def ^:const animation-secs 0.2)

(defn decelerate
  "this is an unintended jerk function ;)"
  [velocity]
...
(defn get-acceleration
  "This is how the deceleration should have been done"
  [velocity]  
...
(defn particle-update 
  [{:keys [delta-time] :as game} {:keys [player-x player-y] :as state}]
  (let [x-vel (get-x-velocity state)]))
